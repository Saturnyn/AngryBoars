...
Entity.prototype = {
	...
	render: function(){
		...
		}else if(this.t==BUSH || this.t==CRATE){
			if(this.destroyed){
				s = this.skin2;
			}
		}
...
var CAT = "c", PLAYER = "p", BOAR="b", ATTACK = "a", BEER = "be", BUSH="bu",CRATE="cr",
...
*/
	var crate = new Entity(CRATE, 18, 23);
	crate.skin = {x:41,y:67,w:28,h:33,dx:6,dy:10};
...
function setCrateRandPos(crate){
	...
	crate.setPos(
		WIDTH*(1+3*Math.random()) + camera,
		(0.2+Math.random()*0.6)*(HEIGHT-crate.h) );
}
...
function tic(){
	...
	var i,j,cat,cat2,dist,dist2,boar,boar2,crate,bush;	
